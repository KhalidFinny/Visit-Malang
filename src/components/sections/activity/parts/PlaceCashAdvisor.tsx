  ...
  return (
    <div className="p-5 md:p-7 xl:p-8 font-sans">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-swiss text-sm font-black tracking-[0.08em] uppercase text-[#A3B18A]">Perencana Uang Tunai</span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-6">
        <div className="rounded-xl border border-[#2D221F]/10 bg-[#faf9f5] p-4 md:p-5">
          <span className="text-swiss text-sm font-black tracking-[0.08em] uppercase text-[#2D221F]/40 block mb-2">Tiket Masuk</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[minmax(200px,240px)_1fr] gap-6 pt-6 border-t border-[#2D221F]/10 items-start">
        <div className="bg-[#f5f4f0] border border-[#2D221F]/10 p-5 md:p-6 rounded-xl flex flex-col justify-center">
          <span className="text-swiss text-sm font-black tracking-[0.08em] uppercase text-[#2D221F]/40 block mb-2">Cadangan Disarankan</span>
        ...
        <div className="space-y-5">
          <div>
            <h5 className="text-swiss text-sm font-black tracking-[0.08em] uppercase text-[#2D221F]/40 mb-2">Catatan Logistik</h5>
      ...
      <div className="mt-5 flex justify-start md:justify-end">
        <button onClick={() => setShowReportForm(!showReportForm)} className="text-sm font-black tracking-[0.08em] uppercase text-[#2D221F]/45 hover:text-[#A3B18A] transition-all cursor-pointer underline underline-offset-4">
          {showReportForm ? "Tutup Formulir" : "Laporkan perubahan harga"}
        </button>
      </div>
      {showReportForm && (
        <form onSubmit={handleSubmitReport} className="mt-5 p-5 rounded-xl bg-[#f5f4f0] border border-[#2D221F]/10 space-y-4">
          <div>
            <h5 className="text-swiss text-sm font-black tracking-[0.08em] uppercase text-[#2D221F] mb-1">Laporkan Koreksi Harga</h5>
            <p className="text-sm md:text-base text-[#2D221F]/60 leading-relaxed font-medium">Masukkan harga yang benar-benar Anda bayar. Laporan akan digabungkan secara otomatis.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[{ label: "Tiket Domestik", v: domesticInput, s: setDomesticInput }, { label: "Tiket Mancanegara", v: foreignInput, s: setForeignInput }, { label: "Biaya Transportasi", v: transportInput, s: setTransportInput }, { label: "Biaya Parkir", v: parkingInput, s: setParkingInput }].map((f, i) => (
              <div key={i}>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-2">
            {submitSuccess ? <span className="text-sm font-black tracking-wider uppercase text-[#A3B18A] font-swiss">Terkirim! Terima kasih!</span> : <span className="text-sm text-[#2D221F]/40 font-semibold font-swiss">Harga digabungkan otomatis</span>}
